...
$projects = searchProjects($conn, $search, $role);
...
<body>
  <div class="dashboard-container">
    <h2>Welcome, <?= htmlspecialchars($_SESSION['full_name']) ?> (Admin Editor)</h2>
        ...
        <?php if (count($projects) > 0): ?>
          <?php foreach ($projects as $p): ?>
            ...
            <tr>
              ...
              <!-- Actions -->
              <td>
                <?php if ($state === 'submitted_to_editor'): ?>
                  <a href="../actions/edit_project.php?id=<?= $p['id'] ?>">Edit</a> |
                  <a href="../actions/update_state.php?id=<?= $p['id'] ?>&state=editor_approved">Submit</a> |
                  <a href="../actions/update_state.php?id=<?= $p['id'] ?>&state=changes_requested_by_editor">Request Changes</a>
                <?php else: ?>
                  <em>Waiting</em>
                <?php endif; ?>
              </td>
