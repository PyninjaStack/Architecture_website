window.addEventListener("DOMContentLoaded", () => {
  ...
  function renderProjects(filterType) {
    ...
    // ✅ Get max number of projects in any metadata group
    let maxGroupSize = 0;
    ...
    // ✅ Decide shrink class based on max group size
    let shrinkClass = "";
    ...
    // ✅ Apply same shrink class to all group stacks for consistency
    wrapper.querySelectorAll(".group-stack-static").forEach(group => {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const toggleContainer = toggleBtn?.closest(".social-toggle-container");
