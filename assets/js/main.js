...
/*window.addEventListener("DOMContentLoaded", () => {
  ...
  function renderProjects(filterType) {
    ...
    // 🔽 Custom sort for status filter
    if (filterType === "status") {
      const statusOrder = ['Idea', 'In Progress', 'UnderConstruction', 'Completed'];
      sortedKeys.sort((a, b) => {
        return statusOrder.indexOf(a) - statusOrder.indexOf(b);
      });
    } else {
      // Default sort (numbers first, then text)
      sortedKeys.sort((a, b) => {
        if (!isNaN(a) && !isNaN(b)) return a - b;
        return a.localeCompare(b);
      });
...
/*window.addEventListener("DOMContentLoaded", () => {
  ...
  const title = document.createElement("div");
  title.className = "project-title";
  title.textContent = `${project.code} - ${project.title}`;
